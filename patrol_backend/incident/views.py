...
from .exports import (
    generate_incident_excel,
    generate_incident_pdf,
    get_incident_export_queryset,
)
...
class IncidentExportExcelView(APIView):
    ...
    def get(self, request):
        ...
        return generate_incident_excel(queryset, request)


class IncidentExportPdfView(APIView):
    """GET /incident/dashboard/export-pdf/ — same fields as Excel, dashboard PDF style."""
    permission_classes = [IsAuthenticated]
    ...
    def get(self, request):
        ...
        if error_response:
            return error_response
        try:
            content, filename = generate_incident_pdf(queryset, request)
