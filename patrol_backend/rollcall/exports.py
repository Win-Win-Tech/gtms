...
def _session_row(session, request, include_site=False):
    ...
    if include_site:
        row.insert(2, session.site.name if session.site else "")
    return row
...
def generate_rollcall_excel(queryset, request, include_site=False):
    wb = Workbook()
    ws = wb.active
    ws.title = "Roll Call"
    ...
    for session in queryset.order_by("-shift_date", "-started_at"):
        ws.append(_session_row(session, request, include_site=include_site))
...
def generate_rollcall_pdf(queryset, request, include_site=False):
    from reportlab.lib import colors
    ...
    for session in sessions:
        row = _session_row(session, request, include_site=include_site)
