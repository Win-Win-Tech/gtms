...
class AttendanceCheckinDashboardV4Serializer(serializers.ModelSerializer):
    """
    V4 Serializer optimized for performance:
    Accepts 'prefetched_logs' in context (grouped by guard_id) to avoid N+1 queries.
    """
    guard_name = serializers.CharField(source="guard.name", read_only=True)
    employee_code = serializers.CharField(source="guard.employee_code", read_only=True)
    user_role = serializers.CharField(source="guard.role", read_only=True)
    ...
    def get_log_pairs(self, obj):
        ...
        # Optimization: use prefetched_logs from context if available
        prefetched_logs = self.context.get('prefetched_logs')
        if prefetched_logs is not None:
            # logs are grouped by guard_id in the view to reduce linear scanning
            guard_logs = prefetched_logs.get(obj.guard_id, [])
            logs = [
                log for log in guard_logs
                if log.assignment_id == obj.assignment_id and 
                   log.shift_id == obj.shift_id and 
                   log.org_location_id == obj.org_location_id and
                   search_start_utc <= log.timestamp < search_end_utc
            ]
            # Since logs are pre-sorted in the query, they remain sorted here.
        else:
            # Fallback to single query if not provided
            logs = CheckInLog.objects.filter(
                guard=obj.guard,
                assignment=obj.assignment,
                shift=obj.shift,
